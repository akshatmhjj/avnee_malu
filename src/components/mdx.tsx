...
function createHeading(as: "h1" | "h2" | "h3" | "h4" | "h5" | "h6") {
  const level = Number.parseInt(as[1]) as 1 | 2 | 3 | 4 | 5 | 6;
  const CustomHeading = ({
    children,
    ...props
...
function createCodeBlock(props: React.ComponentProps<"pre">) {
  ...
  if (typeof children === "object" && children && "props" in children) {
    const nestedProps = children.props as { className?: string; children?: ReactNode };
    const className = nestedProps.className;

    if (className) {
      const codeChildren = nestedProps.children ?? "";
      const rawCode = Array.isArray(codeChildren)
        ? codeChildren.map((item) => (typeof item === "string" ? item : "")).join("")
